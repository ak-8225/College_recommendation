...
export async function GET(req) {
  ...
  if (!phone) {
    return new Response(JSON.stringify({ error: 'Missing phone parameter' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  }
...
export async function POST(req) {
  ...
  if (!phone) {
    return new Response(JSON.stringify({ error: 'Missing phone in body' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  }
  const userCollegeDataStore = await readData();
